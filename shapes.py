...
try:
    ...
    raise Exception(
        "tkinter did not import successfully - check you are running Python 3 and that tkinter is available."
    ) from e
...
class Paper:
    ...
    def __init__(self, width=600, height=600):
        ...
        if Paper.tk is not None:
            raise Exception(
                "Error: Paper has already been created, there can be only one."
            )

        try:
            Paper.tk = Tk()
        except ValueError as e:
            raise Exception("Error: could not instantiate tkinter object") from e

        # Set some attributes
        Paper.tk.title("Drawing shapes")
        Paper.tk.geometry(f"{str(width)}x{str(height)}")
...
class Shape:
    ...
    def __init__(self, width=50, height=50, x=None, y=None, color="black"):
        ...
        if Paper.tk is None:
            raise Exception(
                "A Paper object has not been created. There is nothing to draw on."
            )
        ...
        self.x = (Paper.tk.paper_width / 2) - (self.width / 2) if x is None else x
        self.y = (Paper.tk.paper_height / 2) - (self.height / 2) if y is None else y
